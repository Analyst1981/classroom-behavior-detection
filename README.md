...
仓库地址：https://github.com/Analyst1981/classroom-behavior-detection

> 本项目是对公开技术方案（微信公众号文章《基于YOLO＋deepseek课堂行为智能检测系统》）
> 的**独立功能复刻**，全部代码由本仓库作者编写，用于学习与研究。
> 不包含原文所交付的任何私有源码、数据集与训练权重。
...
| 文档 | 内容 |
|---|---|
| 本文 README | 环境准备、安装、启动、验证、常见问题 |
| [功能介绍](docs/功能介绍.md) | 8 个功能模块详解、检测结果字段、技术架构与数据流、接口清单、运行模式 |
| [原文分析与复刻规格](docs/原文分析与复刻规格.md) | 原文信息提取、逐条对应关系、与原文的差异说明 |

**功能模块**：单图检测 · 批量检测（图片文件夹）· 视频检测（MJPEG 实时流 + H.264 输出）· 摄像头实时检测 ·
AI 智能分析（DeepSeek / Qwen）· 检测记录管理 · PDF 报告导出 · 系统总览。
